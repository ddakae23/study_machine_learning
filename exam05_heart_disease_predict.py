...
Original file is located at
    https://colab.research.google.com/drive/1z7fVMxITw1YJIF6Nz-UImOOnFacgxdy1
"""
...
##데이터 정제 - raw_data에서 ? 제거
clean_data = raw_data.replace('?', np.nan) #nan - 숫자가 아닌값(other number), 데이터값은 float
...
print(scaled_data.describe().T) # T - 세로로 줄바꿈
...
model.add(Dropout(0.25)) # Dropout - 과적합 막기위해 사용
